...
def ask_ai(prompt):
    ...
    if res.status_code != 200:
        return f"❌ خطأ في المحادثة: {res.text}"
    ...
    try:
        ...
        if photo and state == "edit":
            ...
            try:
                ...
                send_message(chat_id, f"❌ حدث خطأ أثناء تعديل الصورة: {e}")
                logging.error("Edit image error: %s", e)
            finally:
        ...
        # ================= GENERATE IMAGE =================
            ...
            try:
                ...
                send_message(chat_id, f"❌ حدث خطأ أثناء إنشاء الصورة: {e}")
                logging.error("Generate image error: %s", e)
            finally:
